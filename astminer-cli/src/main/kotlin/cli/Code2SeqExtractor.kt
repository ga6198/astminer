...
import astminer.paths.Code2SeqPathStorage
...
class Code2SeqExtractor : CliktCommand() {
    ...
    val maxPathHeight: Int by option(
        "--maxH",
        help = "Maximum height of path for Code2Seq"
    ).int().default(8)

    val maxPathWidth: Int by option(
        "--maxW",
        help = "Maximum width of path. " +
                "Note, that here width is the difference between token indices in contrast to the original Code2Seq."
    ).int().default(3)
    ...
    private fun extract() {
        ...
        for (extension in extensions) {
            ...
            val storage = Code2SeqPathStorage(outputDirForLanguage.path, batchMode, batchSize)
